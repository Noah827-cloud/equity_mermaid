...
REM 自动修复Streamlit静态文件
echo [INFO] Checking for Streamlit static files fix...
set "SOURCE_DIR=%TARGET_ROOT%app\streamlit\static"
set "TARGET_DIR=%TARGET_ROOT%streamlit\static"

if exist "%SOURCE_DIR%" (
    echo [INFO] Applying Streamlit static files fix...
    ...
    REM 复制静态文件
    powershell -NoProfile -Command "Copy-Item -Path '%SOURCE_DIR%\*' -Destination '%TARGET_DIR%' -Recurse -Force" >nul 2>&1
    
    if exist "%TARGET_DIR%\index.html" (
        echo [OK] Streamlit static files fixed successfully.
    ) else (
        echo [WARN] Streamlit static files fix may have failed. Please run fix_streamlit_static_compatible.bat manually.
    )
) else (
    echo [INFO] No Streamlit static files found, skipping fix.
)
