...
手动编辑模式通过顶部步骤条和进度条引导完成数据录入。完成任一步骤后，该步骤会被记录为“已访问”，你可以直接点击步骤条上任意已访问的标签快速跳转，无需逐步点击“上一步”。导航按钮仍支持回退/前进/重置，系统会在离开当前步骤前检查是否有未保存修改。

### 6.1 全局能力

- **侧边栏工具集**：包含主页/图像识别/手动编辑快捷导航，「ℹ️ 使用说明」折叠面板（17 项操作步骤与导入提示）、翻译额度管理、导入文件统计与调试开关。
...
- 生成后提供两种输出：
  ...
  2. **交互式 HTML 图**：点击「🔍 全屏查看图表」或「🔍 全屏编辑图表」后调用 `generate_visjs_html` / `generate_fullscreen_visjs_html`，在新窗口展示 vis.js 图谱，包含主题切换、缩放、分层、节点高亮、导出 PNG。
- 所有导出均保证只包含有效实体与关系（会自动过滤被隐藏或未建立关系的节点）。

#### 全屏 vis.js 操作指南

- **打开方式**：在步骤 6 生成图表后点击「🔍 全屏查看图表」或「🔍 全屏编辑图表」，浏览器会弹出新窗口加载交互式视图。
- **折叠工具栏**：右上角工具栏支持展开/收起，提供内边距、层级间距、分组选择、隐藏节点/连线管理、物理引擎切换、布局优化等操作；所有调整即时生效并可随时重置。
- **节点样式调整**：工具栏可设置全局节点尺寸、字体、边框，选中节点后可用箭头按钮、键盘方向键或拖动四角控制点微调位置和大小，`🔓 解除固定`、`📐 智能分布` 等按钮可协助恢复层级布局。
- **边样式与导出**：支持在工具栏中切换直线/平滑/动态等线型与蓝/红/绿/紫/橙/灰六种配色（会按持股比例自动调整深浅），并通过「导出图片」将当前视图保存为 PNG。
- **上下文菜单**：右键节点可执行隐藏、重置尺寸、居中、删除、解除锁定；右键连线可隐藏或删除。所有操作会同步写入隐藏列表，可在工具栏对应面板恢复或清空。
