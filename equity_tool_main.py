...
with col1:
    ...
    if st.button("打开图像识别模式"):
        ...
        st.code(".venv\Scripts\streamlit.exe run enhanced_equity_to_mermaid.py", language="powershell")
    st.markdown('</div>', unsafe_allow_html=True)
...
with col2:
    ...
    if st.button("打开手动编辑模式"):
        ...
        st.code(".venv\Scripts\streamlit.exe run manual_equity_editor.py", language="powershell")
    st.markdown('</div>', unsafe_allow_html=True)
